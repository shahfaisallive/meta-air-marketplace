            ...
            {props.galleries.map((item, index) => (
                <div key={item.artId} className='w-full h-auto grid grid-cols-1 bg-[#161A42] border-2 border-[#161A42] dark:bg-white rounded-2xl text-white dark:text-gray-800 dark:border-2 dark:border-gray-200'>
                    ...
                    <div className="border-t-2 border-[#2E357B] dark:border-gray-200 p-4">
                       {item.sold?"":<button className='rounded-full bg-gradient-to-b from-[#3461FF] to-[#8454EB] text-white text-base w-full px-4 py-2 shadow-md' 
                    //    onClick={()=>props.children.buyFunction(item)} 
                       ><Link href={`/market/${item.itemId}?id=${item.itemId}`}><a>⚡ {item.buttonTitle}</a></Link> </button>} 
                    </div>
                </div>
            ))}
        </div>
