...
export default function SingleNftItem() {
  ...
  useEffect(() => {
    ...
    loadTokenContractHandler()
    web3Api.web3 && LoadContracts();
  }, [uniqid, uniqtoken, web3Api.web3, isLoading]);

  useEffect(()=>{
    if(marketContract && singleItem){
      getTopBid()
    }
  },[marketAddress, singleItem])
    ...
    try {
      if (priceFromWei) {
        ...
        const result = await marketContract.methods
          .buyNFt(nftAddress, convertIdtoInt, Web3.utils.toWei(nftItem.price, 'ether'))
          .send({ from: account });
        closeLoaderModal();
  ...
  const loadTokenContractHandler = async () => {
    try{
      const tokenContractFile = await fetch("/abis/EngToken.json");
    const convertTokenContractFileToJson = await tokenContractFile.json();
    const tokenAbi = convertTokenContractFileToJson.abi;

    const netWorkId = 4;

    const tokenNetworkObj = convertTokenContractFileToJson.networks[netWorkId];
          // console.log(tokenContractFile)

    console.log("tokenNetworkObj",tokenNetworkObj);
    if (tokenNetworkObj) {
      const tokenAddress = tokenNetworkObj.address;
      ...
      console.log("Error while setting Token Contract",err)
    }
  };

  var getTopBid = async()=>{
    const bid = await marketContract.methods.bids(singleItem.itemId).call()        
    setTopBid(bid)     
  }
  return (
    <>
        ...
        <div className="w-full 2xl:max-w-screen-2xl h-auto pt-[104px] m-auto">
          <div className="flex flex-col mx-8 sm:mx-16 lg:mx-[9vw] space-y-6 py-12">
            ...
            <div className="flex flex-col space-y-12">
              {/* art details */}
              <ArePreview>
                {{ item: singleItem, buyFunction: buyNFT ,account:account, tokenContract:tokenContract, marketContract: marketContract, topBid: TopBid, nftContract: nftContract }}
              </ArePreview>
